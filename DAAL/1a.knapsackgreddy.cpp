...
double fractionalKnapsack(vector<Item>& items, int cap) {
    ...
    for (int i = 0; i < n && cap > 0; i++) {
        if (cap >= items[i].weight) {
            ...
            cout << setw(4) << i + 1 << setw(7) << items[i].weight << setw(7) << items[i].value << setw(10) << fraction << setw(9) << items[i].value << endl;
            profit += items[i].value;
            cap -= items[i].weight;
        } else {
            double fraction = (double)cap / items[i].weight; // Select a fraction of the item
            cout << setw(4) << i + 1 << setw(7) << items[i].weight << setw(7) << items[i].value << setw(10) << fraction << setw(9) << fraction * items[i].value << endl;
